...
import { Gear, GearGrade, GearPropType, GearType, ImageOrigin, Potential } from "../../src/maplegear"
...
test("test Gear (de)serialize", () => {
  ...
  gear.icon = new ImageOrigin(3, [4, 5])
  const potentialMap = new Map<GearPropType, number>()
  potentialMap.set(GearPropType.incDEX, 7)
  gear.options.push(new Potential(12345, 15, 150, "test potential", potentialMap))
...
test("test restoreStandardProps()", () => {
  const gear = new Gear()
  gear.standardProps.set(GearPropType.tuc, 9)
  gear.standardProps.set(GearPropType.tradeBlock, 1)
  gear.props.set(GearPropType.tuc, 3)
  gear.props.set(GearPropType.superiorEqp, 1)
  gear.restoreStandardProps()
  expect(gear.props.get(GearPropType.tuc)).toBe(9)
  expect(gear.props.get(GearPropType.tradeBlock)).toBe(1)
  expect(gear.props.get(GearPropType.superiorEqp)).toBeUndefined()
})

test("test getMaxStar()", () => {
  const gear20 = new Gear()
  gear20.type = GearType.glove
  gear20.props.set(GearPropType.reqLevel, 130)
  gear20.hasTuc = true
  expect(gear20.getMaxStar).toBe(20)
  const gear15 = new Gear()
  gear15.type = GearType.belt
  gear15.props.set(GearPropType.reqLevel, 150)
  gear15.props.set(GearPropType.superiorEqp, 1)
  gear15.hasTuc = true
  expect(gear15.getMaxStar).toBe(15)
  const gear25 = new Gear()
  gear25.type = GearType.breathShooter
  gear25.props.set(GearPropType.reqLevel, 200)
  gear25.hasTuc = true
  expect(gear25.getMaxStar).toBe(25)
  gear25.props.set(GearPropType.onlyUpgrade, 1)
  expect(gear25.getMaxStar).toBe(0)
})

test("test diff()", () => {
  const gear = new Gear()
  gear.props.set(GearPropType.incINT, 120)
  gear.props.set(GearPropType.incLUK, 20)
  gear.props.set(GearPropType.tradeBlock, 2)
  gear.standardProps.set(GearPropType.incINT, 40)
  gear.standardProps.set(GearPropType.incLUK, 30)
  gear.standardProps.set(GearPropType.tradeBlock, 1)
  expect(gear.diff).toBe(120 - 40 + 20 - 30)
})

test("test createFromID()", () => {
  { // 앱솔랩스 아처슈즈
    const gear = Gear.createFromID(1073033)
    expect(gear.itemID).toBe(1073033)
    expect(gear.name).toBe("앱솔랩스 아처슈즈")
    expect(gear.getPropValue(GearPropType.reqLevel)).toBe(160)
    expect(gear.getBooleanValue(GearPropType.equipTradeBlock)).toBe(true)
    expect(gear.maxStar).toBe(25)
    expect(gear.star).toBe(0)
  }
  { // 아케인셰이드 튜너 (PC방)
    const gear = Gear.createFromID(1213030)
    expect(gear.getPropValue(GearPropType.reqLevel)).toBe(200)
    expect(gear.grade).toBe(GearGrade.legendary)
    expect(gear.options.length).toBe(3)
    expect(gear.options[0].convertSummary).toBe("보스 몬스터 공격 시 데미지 : +35%")
    expect(gear.options[1].convertSummary).toBe("데미지 : +12%")
    expect(gear.options[2].convertSummary).toBe("크리티컬 확률 : +12%")
  }
})
