...
export default class SetItem {
    /** 세트 아이템 ID */
    @serializable setItemID = 0;
    /** 세트 아이템 이름 */
    @serializable setItemName = "";
    /** 럭키 아이템 적용 가능 여부 */
    @serializable jokerPossible = true;
    /**
     * 세트 아이템에 포함되는 장비 ID 목록
     *
     * 현재 포함된 아이템은 `true`; 포함되지 않은 아이템은 `false`
     */
    @serializable(esMap()) itemIDs: Map<number, boolean>;
    /** 세트 아이템 개수 별 효과 */
    @serializable(esMap(esMap())) effects: Map<number, Map<GearPropType, number>>;
    ...
    static createFromID(setItemID: number): SetItem | undefined {
        ...
        for(const [count, effects] of Object.entries(data.Effect)) {
            ...
            for(const [key, value] of Object.entries(effects)) {
                props.set(asGearPropType(key), value);
            }
            setItem.effects.set(Number(count), props);
        }
        return setItem;
    }
