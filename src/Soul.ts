...
import { getGearPropString } from "./GearStringHelper";
...
export default class Soul {
    /** 몬스터 이름 */
    @serializable mobName: SoulMobName = "" as SoulMobName;
    /** 소울 이름 */
    @serializable soulName = "";
    /** 스킬명 */
    @serializable skillName = "";
    /** 속성 */
    @serializable(esMap()) option: Map<GearPropType, number>;
    /** 속성 문자열 */
    @serializable optionString = "";
    ...
    /**
     * 소울 충전량으로 증가하는 공격력/마력 증가량을 계산합니다.
     * @param soulCharge 소울 충전량
     * @param soul 장비에 적용된 소울; 소울 인챈터만 적용된 경우 `undefined`
     * @returns 공격력/마력 증가량
     */
    static getChargeAD(soulCharge: number, soul?: Soul): number {
        if(soul && soul.mobName in soulData) {
            if(SoulHelper.getSoulGrade(soul.mobName as SoulMobName) <= 6) {
                return Math.floor(Math.min(500, soulCharge) / 100) * 4;
            }
            return Math.floor(Math.min(500, soulCharge) / 100) * 3;
        }
        return Math.floor(Math.min(500, soulCharge) / 100) * 2;
    }
...
class SoulHelper {
    ...
    static getSoulOption(mobName: SoulMobName, soulType: SoulType): { option: Map<GearPropType, number>, optionString: string } {
        const propMap = new Map<GearPropType, number>();
        const type = SoulHelper.getSoulOptionType(mobName, soulType);
        const value = SoulHelper.getSoulOptionValue(mobName, soulType);
        const optionString = getGearPropString(type, value);
        propMap.set(type, value);
        return { option: propMap, optionString: optionString };
    }
