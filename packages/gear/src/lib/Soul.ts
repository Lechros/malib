...
export class Soul {
  ...
  static createFromName(
    ...
    const { option, optionString } = SoulHelper.getSoulOption(
      mobName,
      soulType
    );
    soul.option = option;
...
class SoulHelper {
  ...
  static getSoulOption(
    mobName: SoulMobName,
    soulType: SoulType
  ): { option: Map<GearPropType, number>; optionString: string } {
  ...
  static getSoulOptionType(
    mobName: SoulMobName,
    soulType: SoulType
  ): GearPropType {
    const isRate: boolean = this.getSoulGrade(mobName) === 1;
    switch (soulType) {
      case SoulType.EX_PAD:
        return isRate ? GearPropType.incPADr : GearPropType.incPAD;
      case SoulType.EX_MAD:
        return isRate ? GearPropType.incMADr : GearPropType.incMAD;
      case SoulType.EX_ALL:
        return isRate ? GearPropType.statR : GearPropType.incAllStat;
      case SoulType.EX_MHP:
        return GearPropType.incMHP;
      case SoulType.EX_CR:
        return GearPropType.incCr;
      case SoulType.EX_IMD:
        return GearPropType.imdR;
      case SoulType.EX_BD:
        return GearPropType.bdR;
      case SoulType.EX_AS:
        return GearPropType.incAllskill;
      case SoulType.STR:
      ...
      case SoulType.ALL:
        return GearPropType.incAllStat;
      case SoulType.PAD:
  ...
  static getSoulOptionString(type: GearPropType, value: number): string {
    switch (type) {
      ...
      case GearPropType.incAllskill:
        return `모든 스킬레벨 : +${value}(5차 제외, 스킬의 마스터 레벨까지만 증가)`;
      default:
        return "";
    }
  }
